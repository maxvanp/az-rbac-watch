...
def scan(
    ...
    if orphans_only and policy is not None:
        console.print("[bold red]Error[/bold red]: --orphans-only and --policy are mutually exclusive.")
        raise typer.Exit(code=2)

    if orphans_only and not tenant_id:
        console.print("[bold red]Error[/bold red]: --orphans-only requires --tenant-id / -t.")
        raise typer.Exit(code=2)
